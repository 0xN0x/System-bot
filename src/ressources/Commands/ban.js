exports.run = (client, message) => {
	if(!message.guild.me.hasPermission("BAN_MEMBERS")) return system.shortsend(message.channel, `I need the \`Ban members\` permission for do that action.`);
	...
	message.mentions.users.map(u => {
		...
		message.channel.send(`Banned **${u.username}**.`);
	});
	return true;
};
