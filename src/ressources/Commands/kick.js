exports.run = (client, message) => {
	if(!message.guild.me.hasPermission("KICK_MEMBERS")) return system.shortsend(message.channel, `I need the \`Kick members\` permission for do that action.`);
	...
	message.mentions.users.map(u => {
		...
		message.channel.send(`Kicked **${u.username}**.`);
	});
	return true;
};
