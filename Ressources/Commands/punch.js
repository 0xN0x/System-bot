exports.run = (client, message, args) => {
  ...
  function selfpunch(channel, sender, receiver) {
    ...
    /*return channel.send({
      embed: {
        ...
        description: `<@${receiver.id}>, you got punched by... yourself ..?`
      }
    })*/
    return channel.send({
      file: {
  ...
  if (message.mentions.users.first() && message.mentions.users.first().id === message.author.id) return selfpunch(message.channel, message.author, message.mentions.users.first())
